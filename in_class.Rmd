...
# modeling 
#install.packages(c('caret', 'ISLR', 'ROCR'))
...
dat_model_log %>%
  ggplot(aes(x = balance)) + 
  geom_point(aes(y = default)) + 
  geom_line(aes(y = logistic_reg), color='blue')
```
...
dat_model_log %>%
  ggplot(aes(x = balance)) + 
  geom_point(aes(y = default)) + 
  geom_line(aes(y = logistic_reg), color="red")
```
...
```{r}
predictions = predict(model_log, data = dat, type="response")
head(predictions)
```
...
```{r}
# Model on Training data = dat_train
